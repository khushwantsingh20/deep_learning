...
STATICFILES_STORAGE = (
    "django_site.manifest_storage.ManifestStaticFilesExcludeWebpackStorage"
)
INLINECSS_CSS_LOADER = "django_inlinecss.css_loaders.StaticfilesStorageCSSLoader"
# ----------------------------------------------------------------------------------------------------------------------
# SECRET_KEY
...
SERVER_EMAIL = "bookings@mylimomate.com.au"

# See https://docs.djangoproject.com/en/3.0/ref/settings/#default-from-email
DEFAULT_FROM_EMAIL = "admin@mylimomate.com.au"
...
USE_SENTRY = _strtobool(get_env_setting("SENTRY_ENABLED", "True"))
SENTRY_DSN_PYTHON = get_env_setting("SENTRY_DSN_PYTHON", None)

if USE_SENTRY and not SENTRY_DSN_PYTHON:
    raise ImproperlyConfigured(
        "SENTRY_DSN must be set unless SENTRY_ENABLED is set to False"
    )
...
SENTRY_CONFIG_JS = {"environment": None, "dsn": SENTRY_DSN_JS}
...
# We need to use the env var provided by heroku for redis
CELERY_BROKER_URL = get_env_setting("REDIS_URL")
