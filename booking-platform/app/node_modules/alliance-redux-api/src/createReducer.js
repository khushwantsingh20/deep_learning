...
export function stringify(obj) {
    ...
    if (obj && obj.prototype && obj.prototype[Typed.label]) {
        return obj.prototype[Typed.label];
    }
...
export default function createReducer(constructor, handlers, transformInitialState = data => data) {
    ...
    return (state = null, action) => {
        ...
        if (state == null) {
            invariant(
                constructor,
                'You must provide a constructor if this is a reducer ' +
                    'that defines the state shape'
            );
            // Provide initial state
            currentState = new constructor();
            // TODO: Allow List & Map as well. Need to get Map support into
            // typed-immutable see https://github.com/Gozala/typed-immutable/pull/19
            invariant(
                isValidStateObject(currentState),
        ...
        invariant(
            nextState != null,
            `Reducer returned ${nextState === null ? 'null' : 'undefined'}. ` +
                `Did you forget to return for reducer handling action '${action.type}'?`
        );

        // Do relaxed check on constructor to help catch issues where you return the wrong
        // thing.
        const looksLikeSameConstructor =
            nextState instanceof validStateConstructor ||
            stringify(nextState) === stringify(validStateConstructor);
        ...
        if (!looksLikeSameConstructor) {
            throw new Error(
                'Reducers must return instance of provided stateConstructor. Using immutable ' +
                    ...
                    `${stringify(validStateConstructor)} but instead got ${stringify(nextState)}`
            );
        }
