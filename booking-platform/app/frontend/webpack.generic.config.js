...
function getVersion() {
    try {
        ...
        console.warn('Git not found, using default version');
        return 'dev'; // Fallback version
    }
}
function getClientEnvironment(publicUrl) {
    ...
    return {
        ...
        __DEBUG__: nodeEnv === 'development' && !argv.no_debug,
...
module.exports = ({
    ...
    // path for djrad variables overrides, different for app vs styleguide.
    djRadImportStr = './src/styles/partials/_djrad-vars.less',
} = {}) => {
...
`);
    ...
    if (isDev) {
        conf.resolve.alias['react-dom'] = '@hot-loader/react-dom';
        conf.plugins.push(new WatchMissingNodeModulesPlugin(path.resolve('../node_modules')));
        if (hotReload) {
            conf.plugins.push(
                ...[
                    // Enables HMR
    ...
    if (antd) {
        ...
        const LogAntdIconPlugin = require('./plugins/LogAntdIconPlugin');
        babelPluginsCommon.push(LogAntdIconPlugin);
    }
    ...
    // Binary files
    conf.module.rules.push({
        test: /\.(jpg|jpeg|png|gif|eot|svg|ttf|woff|woff2|webp)$/,
                ...
                options: {
                    ...
                    plugins: [
                        new LessModifyPreprocessorPlugin({
                            test: /djrad\/styles\/_variables\.less/,
                            apply(source) {
