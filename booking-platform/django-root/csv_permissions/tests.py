...
USER1_MODEL = ClientUser
USER1_TYPE = UserType.choices[UserType.CLIENT]

USER2_MODEL = StaffUser
USER2_TYPE = UserType.choices[UserType.STAFF_MANAGER]
...
class CsvRulesTests(TestCase):
    def test_permissions_parse(self):
        ...
        with override_csv_permissions(csv_data):
            user1 = ClientUserFactory(email="user1@localhost.test")
            user2 = StaffFactory(
                user_type=UserType.STAFF_MANAGER, email="user2@localhost.test"
            )
    ...
    def test_all_global_permission_error(self):
        ...
        with override_csv_permissions(csv_data):
            user = ClientUserFactory(email="user@localhost.test")
    ...
    def test_non_existent_permission_error(self):
        ...
        with override_csv_permissions(csv_data):
            ...
            with warnings.catch_warnings():
                warnings.filterwarnings("ignore", "No implementation of testmodela_own")
                self.assertEqual(
    ...
    def test_yes_permission(self, access_level_yes_mock):
        ...
        with override_csv_permissions(csv_data):
            user = StaffFactory(
                user_type=UserType.STAFF_MANAGER, email="user@localhost.test"
            )
