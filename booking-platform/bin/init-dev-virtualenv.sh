#!/bin/bash
set -o errexit
#set -o nounset  # can't set this because virtualenvwrapper fails
set -o pipefail
...
function help() {
	...
	echo "   --no-dev   - skip installing src-dev.txt requirements"
...
# have had issues with 1.3+ lockfiles are not fully backwards compatible, so this is the minimum now
if [[ $python_bin != "" ]] ; then
...
function print_target_details() {
	...
	echo "poetry ver:   ${poetry_ver} (require min $poetry_ver_min)"
...
if $use_pyenv  ; then
	...
	pyenv virtualenvwrapper || fail "Error: if you got a hook_loader error here, run `pip install virtualenv virtualenvwrapper`"
...
if $use_pyenv ; then
	...
	elif [ $( which pyenv ) != '/usr/local/bin/pyenv' ] && [ $(which pyenv) != '/opt/homebrew/bin/pyenv' ] ; then
...
echo "---------------------------------"
echo "Syncing virtualenv $target_venv using $python_bin ($python_ver) in $target_dir"

venv_args=()


if true ; then
	# for now we have worked around this by not installing python-pip & python3-pip but
	# this is left here for future reference in case you deploy to a platform where they are present
	if [[ $( lsb_release --id --short 2>/dev/null ) = "Ubuntu" ]] &&
		[[ $( lsb_release --release --short | sed -E 's/^([0-9]+).*/\1/' ) -lt 20 ]] ;
	then
		# if the python-pip/python3-pip package is installed then it overrides the built-in python pip
		# but there's a bug in ubuntu versions prior to 20.04:
...
if [[ ! -f $target_dir/bin/activate ]] ; then
	...
	set -x
		$python_bin -m venv "${venv_args[@]}" "$target_dir"
	{ set +x; } 2>/dev/null
else
	...
	if [[ $check_ver_short != $python_ver_short ]]; then
		...
		set -x
			rm -rf "$target_dir"
			python$python_ver_short -m venv "${venv_args[@]}" "$target_dir"
		{ set +x; } 2>/dev/null
	else
...
# now that we know pip will always be available, re-run the poetry version check
poetry_version_check

echo "---------------------------------"
echo "Installing packages into $target_venv"
set -x
	# needed to work around the fact that Debian/Ubuntu have split these out
	# from the core python packages. By installing our own version we overwrite
	# the debian/ubuntu version
